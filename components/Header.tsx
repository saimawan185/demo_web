  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 border-b transition-all duration-500 ${
        scrolled
          ? "bg-ink-950/97 backdrop-blur-xl border-rose-400/25"
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16 md:h-[4.5rem]">
          <Link href="/" className="group">
            <span className="font-display text-2xl md:text-[1.75rem] text-rose-300 italic transition-colors group-hover:text-rose-400">
              Diana&apos;s
            </span>
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href} className="nav-link text-ink-300 hover:text-rose-300 transition-colors">
                {link.label}
              </Link>
            ))}
        ...
        {open && (
          <nav className="md:hidden pb-5 border-t border-white/10 pt-4 flex flex-col gap-3 animate-menu-in">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href} className="text-ink-100 hover:text-rose-300 py-2 text-sm tracking-[0.14em] uppercase" onClick={() => setOpen(false)}>
                {link.label}
              </Link>
            ))}
